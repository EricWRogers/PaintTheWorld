...
public class LandingSplashEmitter : MonoBehaviour
{
    ...
    public void EmitPuddle(int stacks)
    {
        ...
        // center stamp
        StampAtWorldPoint(transform.position, splashRadius, color);

        // ring stamps to make a big puddle
        for (int ring = 1; ring <= rings; ring++)
        {
            ...
            for (int i = 0; i < samplesPerRing; i++)
            {
                ...
                StampAtWorldPoint(transform.position + offset, splashRadius, color);
            }
        }
    }

    void StampAtWorldPoint(Vector3 worldPoint, float splashRadius, Color color)
    {
